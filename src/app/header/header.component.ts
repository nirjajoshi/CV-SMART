...
export class HeaderComponent implements OnInit {
  ...
  login() {
    ...
    this.http.post<LoginResponse>('http://localhost:8000/api/v1/users/login', this.loginData).subscribe(
      (response) => {
  ...
  logout() {
    if (this.userDetails && this.userDetails.id) {
      this.http.post<LogoutResponse>('http://localhost:8000/api/v1/users/logout', { userId: this.userDetails.id }).subscribe(
        (response) => {

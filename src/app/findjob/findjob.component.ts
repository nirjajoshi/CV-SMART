...
export class FindjobComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.http.get<{ userId: string }>(`http://localhost:8000/api/v1/userid/get-user-id?email=${this.userEmail}`)
        .subscribe(
            (response: { userId: string }) => {
                ...
                this.http.post('http://localhost:8000/api/v1/resume/uploadResume', formData)
                    .subscribe(
                        (response: any) => {
  ...
  fetchJobs(): void {
    ...
    const apiUrl = `http://localhost:8000/api/v1/matching-jobs?userId=${userId}&commonId=${commonId}`;
